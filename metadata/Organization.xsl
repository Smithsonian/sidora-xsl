...
<xsl:template match="/">
  ...
  <dl class="metadata-table">
    ...
    <xsl:for-each select="//cpf:nameEntry[@localType='alternative']/cpf:part">
    <xsl:if test="current()!=''">
    ...
    <dt>Address Line 1</dt><dd><xsl:value-of select="current()"/></dd>
      ...
      <xsl:for-each select="//cpf:cpfRelation/cpf:relationEntry">
          <xsl:if test="current()!=''">
              <dt>Relation:  </dt><dd><xsl:value-of select="current()"/></dd>
          </xsl:if>
      </xsl:for-each>
