...
<xsl:stylesheet version="1.0" 
    ...
    <xsl:template match="SIdoraConcept">
        ...
        <xsl:for-each select="relationship">
            <xsl:if test="current()!=''  and (value !='' or note != '')">
                <xsl:call-template name="formatTypeValueNote">
                    <xsl:with-param name="text" select="'Related Item Type'"/>
                </xsl:call-template>
        ...
        <xsl:for-each select="actor">
            <xsl:if test="@role !='' or *[normalize-space()] ">
                ...
                <dt><xsl:text>Actor: </xsl:text>
                    ...
                    <xsl:if test="@id != ''">
                        <dd><xsl:text>ID: </xsl:text><xsl:value-of select="@id"/></dd>
                    </xsl:if>
    ...
    <xsl:template name="formatTypeValueNote">
        ...
        <xsl:for-each select="@id">
            <xsl:if test="current() != ''">
                <dd><xsl:text>Identifier: </xsl:text><xsl:value-of select="."/></dd>
            </xsl:if>
        </xsl:for-each>
    ...
    <xsl:template name="formatTaxonName">
        <xsl:if test="@scheme != ''">
            <dd><xsl:text>Taxonomic Thesaurus: </xsl:text><xsl:value-of select="@scheme"/></dd>
        </xsl:if>
        <xsl:if test="genus !=''">
